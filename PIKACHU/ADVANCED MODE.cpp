...
void PaintBroad_Advanced(str_linkedList*& list, int** c, int height, int width, matrix Matrix)
{
    ...
    for (int i = 1; i <= height; i++)
    {
        ...
        for (int j = list[i].width + 1; j <= width; j++)
        {
            c[i][j] = -1;
            Matrix.val[i][j].data = -1;
        }
    }
}

//Cập nhật các ô đã được click chọn
void PickCell_Advanced(str_linkedList*& list, int** c, int width, int height, int& countcell, matrix& Matrix) // Biến countCell: lưu tổng số ô còn lại
{
    // Khởi tạo kích thước font và ô chữ
